...
## Project Reflection
I got started on my final coding project somewhat late in the semester and didn't keep a record of the process as I was working on it. Instead, I will summarize some of the challenges that I faced here and reflect on the over all process of creating the project.

### Chosen Language and Database Decisions
I had initially intended to develop the project in C# using Blazor or ASP.NET (since C# is the language I'm most familiar with), but I've never used C# outside of the context of Unity, and I didn't give myself enough time to feel confident with deploying a Blazor or ASP.NET app, so decided to use TypeScript instead. I chose TypeScript over JavaScript because I prefer statically typed languaged. I feel like JavaScript obfuscated object types too much, and I don't like working with objects without having a very clear idea of chat they are. TypeScript is easy to get up and running, easy to deploy on github pages, and offers static typing to help mitigate some of the grievances I have with working in vainlla JavaScipt.

For the database, I chose to go with IndexedDB. I chose this over other implementations becaue I knew it would be easy to get up and running without having to deal with remote connections.

### Challenges
The primary challenge I faced during my work on this project was figuring out how to do things in TypeScript which I can do in C#, and pivoting when the implementations which I'm used to don't work how I'm expecting. Two prominent examples of this were in my implementation of Singletons and in the way that I was casting the results of my database queries into objects.

#### Singletons
In C#, if a class inherits from a superclass which defines a generic static member field (for example, static T Instance), every class inheriting from this superclass will have it's own distinct implementation of that static field. For example, if I do something like this:
...
LoginService.Instance will point to an instance of LoginService and UserService.Instance will point to an instance of UserService. Even though the static _instance field belongs to Singleton, each subclass implementing a new generic instance of Singleton will have its own static _instance member.

In Typescript, however, this appears not to be the case. In my attempt to implement a singleton pattern like the, I realized that the first class I called .Instance on was caching a reference to itself in _instance and that no additional singleton instances were created. In other words, if I called LoginService.Instance first and UserService.Instance second, UserService.Instance would return a reference to the LoginService since the LoginService was cached in the _instance field first and the field is no longer null.

#### Casting Database Results
In my initial implementation of my Repository classes, I was handling the result of the EventTarget in  ```cursorRequest?.addEventListener``` by casting it to the deired class (i.e., ```const task = cursor.value as Task```). Doing this in C#, the result is a true instance of the Task class, with access to all of that class's methods and variables. Apparently this is not the case in TypeScript. When I tried to invoke task.IsOverdue() on the task classes I was receiving and casting from the database, I kept getting an error that the method didn't exist. This was such a frustrating issue that I eventually had to turn to copilot to resolve it (that conversation can be found here: https://copilot.microsoft.com/shares/uyZawt3i8e5dBe4neqFcv). Apparently, casting cursor.result as Task wasn't actually creating a Task object but just a data object with the variables which one would expect from a Task. The methods, however, were not there. I ended up resolving the issue by creating a getTaskFromAny method in my repository which constructs a new task by feeding the result of the database query into the constructor of my Task object.

---
Beyond these issuues, most of the implementation of the features of the app was fairly straightforward, and only required googling and finding resources on stack overflow for things like iterating through enums and creating Maps with initial values. References to these resources can be found in the project documentation.

### Architecture 
I admittedly went into the creation of the app without a solid idea of how I wanted to structure it. I think this proved to be a mistake in the end, speficically with regard to how I am relaying information between the database and the ui.

The way I handled it was by implementing callback methods which are defined in my ui classes, passed into my service classes, then passed into and invoked in the repository when the data is received from the database. Functionally, that looks something like this:

```typescript
// in ui script
service.getTasks((success, msg, tasks) => {
    if (success) { this.tasks = tasks; }
    console.log(msg); 
}) 

// in service
getTasks(callback: (success: bool, msg: string, tasks: Task[]) => void) {
    // do user authentication
    repo.getTasks(user, callback);
}

// in repo
getTasks(user: string, callback: (success: bool, msg: string, tasks: Task[]) => void) {
    // get the result
    callback(true, "Tasks successfully retrieved from database", taskList);
}
```

When I first implemented my database functions, I used these callback methods to account for the asynchronous nature of the database calls. And while it is successful at that, and the app does function as intended, I feel I would have been better served to have implemented a ViewModel class to hold the tasklist rather than keeping the list in the ui class. Then I could have used the Observer pattern to subscribe the ui to the ViewModel class when the tasklist gets updated, and I could avoid all the callback methods that I'm using in the current implementation. If I made the ViewModel a singleton, the ui class and the repository could both access it easily, and the repository could simple call ```viewModel.setTasks(taskList);``` when the database is finished processing. This would have made the implementation much more streamlined.
